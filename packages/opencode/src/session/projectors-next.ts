...
export default [
  SyncEvent.project(SessionEventSync.AgentSwitched, (db, data, event) => {
  ...
  SyncEvent.project(SessionEventSync.ModelSwitched, (db, data, event) => {
    db.update(SessionTable)
      .set({
        model: data.model,
        time_updated: DateTime.toEpochMillis(data.timestamp),
